...
export function CollegeCompare({
  ...
  return (
    <main className="flex-1 flex flex-col min-w-0 overflow-hidden bg-background">
      ...
      <div className="bg-surface-container-lowest border-b border-outline-variant px-gutter py-3 flex flex-wrap items-center gap-3 sticky top-0 z-40 flex-shrink-0 shadow-sm">
        <div className="flex-1 min-w-[200px] max-w-md relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-on-surface-variant size-5" />
          <input
            className="w-full pl-10 pr-4 py-2 h-[38px] bg-surface-container-low border border-outline-variant rounded text-sm focus:outline-none focus:ring-1 focus:ring-primary focus:border-primary"
            placeholder="Search participating colleges..."
            type="text"
            value={searchQuery}
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {(collegeTypeFilter.length > 0 || stateFilter.length > 0 || nirfFilter || searchQuery) && (
            <button
              ...
              className="text-[11px] font-bold text-[#982b35] hover:underline shrink-0 ml-1"
            >
              Clear all
            </button>
          )}
        ...
        <div className="ml-auto shrink-0 flex items-center">
          <p className="text-[11px] font-bold text-on-surface-variant uppercase tracking-widest">
            <span className="text-[#982b35] text-base tabular-nums font-extrabold">{filteredColleges.length}</span> colleges
          </p>
        </div>
      </div>
